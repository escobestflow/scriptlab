...
export function Studio({
  ...
  // ── Hero gradient configurator (desktop-only internal tool) ────
  // State for the right-side flyout that controls the 3-layer
  // gradient overlay on .v2-desktop-hero-image. The current
  // gradient is applied to the image as a `--hero-gradient-bg`
  // custom property; the CSS rule `background-image: var(...)`
  // reads it back. Persisted to localStorage so iterating on
  // values across page reloads keeps the user's tweaks.
  const HERO_GRAD_STORAGE_KEY = "unfold:hero-grad-cfg-v1";
  const [heroGradOpen, setHeroGradOpen] = useState(false);
  const [heroGradient, setHeroGradient] = useState<HeroGradientConfig>(() => {
    if (typeof window === "undefined") return DEFAULT_HERO_GRADIENT;
    try {
      const raw = window.localStorage.getItem(HERO_GRAD_STORAGE_KEY);
      if (!raw) return DEFAULT_HERO_GRADIENT;
      const parsed = JSON.parse(raw);
      // Defensive deep-merge: missing nested fields fall back to defaults
      // so older saved configs still load cleanly after we add a field.
      return {
        radial: { ...DEFAULT_HERO_GRADIENT.radial, ...(parsed?.radial ?? {}),
          inner: { ...DEFAULT_HERO_GRADIENT.radial.inner, ...(parsed?.radial?.inner ?? {}) },
          outer: { ...DEFAULT_HERO_GRADIENT.radial.outer, ...(parsed?.radial?.outer ?? {}) },
        },
        right: { ...DEFAULT_HERO_GRADIENT.right, ...(parsed?.right ?? {}),
          start: { ...DEFAULT_HERO_GRADIENT.right.start, ...(parsed?.right?.start ?? {}) },
          end:   { ...DEFAULT_HERO_GRADIENT.right.end,   ...(parsed?.right?.end   ?? {}) },
        },
        left: { ...DEFAULT_HERO_GRADIENT.left, ...(parsed?.left ?? {}),
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem(HERO_GRAD_STORAGE_KEY, JSON.stringify(heroGradient));
    } catch { /* quota / private mode — ignore */ }
  }, [heroGradient]);
  // Memo-style derived CSS string. Recomputed on every render but
  ...
  return (
    <PartnerStoryContext.Provider value={{
      ...
      >
          ...
          <div className="v2-desktop-hero">
            ...
            <div className="v2-desktop-hero-meta">
              ...
              <div className="v2-desktop-hero-updated">
                ...
                {/* Hero-gradient configurator CTA. Opens the right-side
                    flyout below where every parameter of the 3-layer
                    overlay can be tweaked live. Internal tool — visible
                    on desktop so the user can communicate gradient
                    adjustments back. */}
                <button
                  type="button"
                  className="v2-desktop-hero-grad-cta"
                  onClick={() => setHeroGradOpen(true)}
                  aria-label="Configure hero image gradient"
                  title="Configure hero image gradient"
                >
        ...
        {/* Right-side flyout panel for tuning the hero gradient.
            Always mounted (visibility controlled by `open` prop so
            the slide-in/out animation runs cleanly). Gated on
            desktop only — the gradient overlay only paints on the
            `.v2-desktop-hero-image` element which itself only
            renders at ≥1440. */}
        {isV2 && isDesktop && (
          <HeroGradientConfigurator
            open={heroGradOpen}
            onClose={() => setHeroGradOpen(false)}
            value={heroGradient}
...
/* ============================================ */
/* ===== HERO GRADIENT CONFIGURATOR =========== */
/* ============================================ */
/* Right-side flyout for tweaking the 3-layer gradient overlay on
   the v2 desktop project-detail hero image. Internal tool — the
   user iterates on values here, then communicates the result
   (read out of the textarea at the bottom of the panel) back into
   the static CSS rule in app/globals.css.

   Every control is wired to the parent's `onChange(next)` callback,
   so React owns the value. The image renders the live preview by
   reading the `--hero-gradient-bg` CSS variable that Studio sets
   on the image button's inline style.

   Control inventory (one of each major type, for "all kinds of
   controls" coverage):
     • Toggle switch  — layer enabled
     • Radio segments — radial shape (ellipse / circle)
     • Number input   — width, height, angle
     • Range slider   — position, alpha, stop %  (each paired with
                        a synced number input)
     • Color picker   — native <input type="color">
     • Read-only textarea — emitted CSS
     • Buttons        — reset to defaults, copy CSS                 */

function HeroGradientConfigurator({
  open,
  onClose,
  value,
  onChange,
  ...
  // Convenience setters for nested updates. Each writes one path
  // through the config tree without losing siblings.
  const setRadial = (patch: Partial<HeroGradientConfig["radial"]>) =>
    onChange({ ...value, radial: { ...value.radial, ...patch } });
  const setRadialInner = (patch: Partial<HeroGradientConfig["radial"]["inner"]>) =>
    onChange({ ...value, radial: { ...value.radial, inner: { ...value.radial.inner, ...patch } } });
  const setRadialOuter = (patch: Partial<HeroGradientConfig["radial"]["outer"]>) =>
    onChange({ ...value, radial: { ...value.radial, outer: { ...value.radial.outer, ...patch } } });
  const setRight = (patch: Partial<HeroGradientConfig["right"]>) =>
    onChange({ ...value, right: { ...value.right, ...patch } });
  const setRightStart = (patch: Partial<HeroGradientConfig["right"]["start"]>) =>
    onChange({ ...value, right: { ...value.right, start: { ...value.right.start, ...patch } } });
  const setRightEnd = (patch: Partial<HeroGradientConfig["right"]["end"]>) =>
    onChange({ ...value, right: { ...value.right, end: { ...value.right.end, ...patch } } });
  const setLeft = (patch: Partial<HeroGradientConfig["left"]>) =>
    onChange({ ...value, left: { ...value.left, ...patch } });
  const setLeftStart = (patch: Partial<HeroGradientConfig["left"]["start"]>) =>
    onChange({ ...value, left: { ...value.left, start: { ...value.left.start, ...patch } } });
  ...
  return (
    <>
      <div className={`hero-grad-scrim ${open ? "open" : ""}`} onClick={onClose} />
      <aside className={`hero-grad-panel ${open ? "open" : ""}`} aria-hidden={!open}>
        <header className="hero-grad-panel-header">
          <h2 className="hero-grad-panel-title">Hero Gradient</h2>
          <div className="hero-grad-panel-header-actions">
            <button
              type="button"
              className="hero-grad-reset"
            ...
            <button
              type="button"
              className="hero-grad-close"
              onClick={onClose}
              aria-label="Close gradient configurator"
            >
        ...
        <div className="hero-grad-panel-body">
          ...
          <section className="hero-grad-section">
            <div className="hero-grad-section-header">
              <h3 className="hero-grad-section-title">Radial Vignette</h3>
              <Switch
                checked={value.radial.enabled}
                onChange={enabled => setRadial({ enabled })}
                label="Enabled"
              />
            </div>

            <div className="hero-grad-row">
              <label className="hero-grad-label">Shape</label>
              <div className="hero-grad-radio-group" role="radiogroup" aria-label="Radial shape">
                {(["ellipse", "circle"] as const).map(s => (
                  <span key={s}>
                    <input
                      type="radio"
                      name="hero-grad-radial-shape"
                      id={`hero-grad-shape-${s}`}
                      checked={value.radial.shape === s}
                      onChange={() => setRadial({ shape: s })}
                    />
                    <label htmlFor={`hero-grad-shape-${s}`}>{s[0].toUpperCase() + s.slice(1)}</label>
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="hero-grad-row">
              <label className="hero-grad-label">Width (px)</label>
              <input
                type="number"
                className="hero-grad-number"
                value={value.radial.width}
                min={0}
                onChange={e => setRadial({ width: numOrZero(e.target.value) })}
              />
            </div>
            <div className="hero-grad-row">
              <label className="hero-grad-label">Height (px)</label>
              <input
                type="number"
                className="hero-grad-number"
                value={value.radial.height}
                min={0}
                onChange={e => setRadial({ height: numOrZero(e.target.value) })}
                disabled={value.radial.shape === "circle"}
              />
            </div>

            <RangeRow
              label="Position X (%)"
              value={value.radial.posX}
              onChange={posX => setRadial({ posX })}
              min={0} max={100} step={0.5}
            />
            <RangeRow
              label="Position Y (%)"
              value={value.radial.posY}
              onChange={posY => setRadial({ posY })}
              min={0} max={100} step={0.5}
            />

            <p className="hero-grad-stop-title">Inner Stop</p>
            <StopControls
              stop={value.radial.inner}
              onChange={setRadialInner}
              idPrefix="hero-grad-radial-inner"
            />

            <p className="hero-grad-stop-title">Outer Stop</p>
            <StopControls
              stop={value.radial.outer}
              onChange={setRadialOuter}
              idPrefix="hero-grad-radial-outer"
            />
          </section>
          ...
          <section className="hero-grad-section">
            <div className="hero-grad-section-header">
              <h3 className="hero-grad-section-title">Right Edge Fade</h3>
              <Switch
                checked={value.right.enabled}
                onChange={enabled => setRight({ enabled })}
                label="Enabled"
              />
            </div>
            <div className="hero-grad-row">
              <label className="hero-grad-label">Angle (deg)</label>
              <input
                type="number"
                className="hero-grad-number"
                value={value.right.angle}
                min={0} max={360}
                onChange={e => setRight({ angle: numOrZero(e.target.value) })}
              />
            </div>
            <p className="hero-grad-stop-title">Start Stop</p>
            <StopControls
              stop={value.right.start}
              onChange={setRightStart}
              idPrefix="hero-grad-right-start"
            />
            <p className="hero-grad-stop-title">End Stop</p>
            <StopControls
              stop={value.right.end}
              onChange={setRightEnd}
              idPrefix="hero-grad-right-end"
            />
          </section>
          ...
          <section className="hero-grad-section">
            <div className="hero-grad-section-header">
              <h3 className="hero-grad-section-title">Left Edge Fade</h3>
              <Switch
                checked={value.left.enabled}
                onChange={enabled => setLeft({ enabled })}
                label="Enabled"
              />
            </div>
            <div className="hero-grad-row">
              ...
              <input
                type="number"
                className="hero-grad-number"
                value={value.left.angle}
                min={0} max={360}
                onChange={e => setLeft({ angle: numOrZero(e.target.value) })}
              />
            </div>
            <p className="hero-grad-stop-title">Start Stop</p>
            <StopControls
              stop={value.left.start}
              onChange={setLeftStart}
              idPrefix="hero-grad-left-start"
            />
            <p className="hero-grad-stop-title">End Stop</p>
            <StopControls
              stop={value.left.end}
              onChange={setLeftEnd}
              idPrefix="hero-grad-left-end"
            />
          </section>

          {/* ── Output / Copy ── */}
          <section className="hero-grad-output">
            <h3 className="hero-grad-section-title" style={{ marginBottom: 8 }}>Output CSS</h3>
            <textarea
            ...
            <button
              type="button"
              className={`hero-grad-copy ${copied ? "copied" : ""}`}
              onClick={copy}
            >
...
/** Reusable toggle-switch input. Mounted on a hidden <input> so the
 *  browser still handles keyboard activation and form-state semantics. */
function Switch({ checked, onChange, label }: {
  ...
  return (
    <label className="hero-grad-switch" aria-label={label}>
      <input
        type="checkbox"
        checked={checked}
        onChange={e => onChange(e.target.checked)}
      />
      <span className="hero-grad-switch-track" aria-hidden="true" />
      <span className="hero-grad-switch-thumb" aria-hidden="true" />
    </label>
  );
}

/** Range slider + number-input pair sharing one value. */
function RangeRow({ label, value, onChange, min, max, step }: {
  label: string;
  value: number;
  ...
  return (
    <div className="hero-grad-row hero-grad-row-stacked">
      <span className="hero-grad-label">{label}</span>
      <div className="hero-grad-range-row">
        <input
          type="range"
          className="hero-grad-range"
          value={value}
          min={min}
          max={max}
          step={step}
...
/** Renders the three controls for a single gradient stop:
 *  color (color picker) + alpha (range 0..1) + position (range 0..100%). */
function StopControls({ stop, onChange, idPrefix }: {
  stop: { color: string; alpha: number; stop: number };
  onChange: (patch: Partial<{ color: string; alpha: number; stop: number }>) => void;
  idPrefix: string;
}) {
  return (
    <>
      <div className="hero-grad-row">
        <label className="hero-grad-label" htmlFor={`${idPrefix}-color`}>Color</label>
        <input
          id={`${idPrefix}-color`}
          type="color"
          className="hero-grad-color"
          value={stop.color}
...
/** parseFloat with a 0 fallback for empty / NaN input. */
